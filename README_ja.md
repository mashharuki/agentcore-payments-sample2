...
デプロイ後、`McpStack` の出力 `McpServerUrl` を `claude mcp add --transport http x402-weather <URL>` に渡せば、AWS上のMCPサーバーに接続できる。**ALBはデフォルトDNS名（カスタムドメインなし）のためHTTP接続になる点に注意**（design.md 7.6節）。
...
| 症状 | 原因・対処 |
|---|---|
| resource server起動直後の最初のリクエストが500になる | facilitatorの起動と競合するタイミング問題。facilitator起動後にもう一度リクエストすれば解消する |
| `CreatePaymentManager` が `Role validation failed` で失敗する | FoundationStack のデプロイ先と admin スクリプトのリージョンが不一致。両方を `us-west-2` に揃える |
| `CreatePaymentManager` が `missing the required permission: bedrock-agentcore:CreateWorkloadIdentity` で失敗する | 自前ロールに base permissions が付いていない。最新の `FoundationStack` を再デプロイする（`ResourceRetrievalRole` に付与済み） |
| `ProcessPayment` / 決済が `Your session has expired` を返す | Payment Sessionが失効。`pnpm payments:admin new-session` で再発行し `.env` の `PAYMENT_SESSION_ID` を更新する |
| 決済後リクエストが本文空の `402 {}` になる | x402 v2 は `PAYMENT-SIGNATURE` ヘッダーを読む。`packages/shared` の最新版を使う |
| ウォレット残高が0のまま | 手順4のPrivyフロントエンドで入金・delegationが完了していない |

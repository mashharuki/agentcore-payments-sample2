...
- `ResourceRetrievalRole`：`bedrock-agentcore.amazonaws.com` を信頼するサービスロール。`CreatePaymentManager` の `roleArn` に渡すため、PaymentManager作成前にCDKで用意しておく（ARNを `CfnOutput` で出力し、管理者スクリプトに渡す）。**信頼ポリシーに加えて identity-based policy（base permissions + per-connector permissions）もCDKで明示付与する**：`CreateWorkloadIdentity` / `DeleteWorkloadIdentity` / `GetWorkloadAccessToken` / `GetResourcePaymentToken`（`token-vault/default*`・`workload-identity-directory/default*`）、`CreatePaymentCredentialProvider` / `GetPaymentCredentialProvider` / `TagResource`、`secretsmanager:GetSecretValue`（`bedrock-agentcore-identity*` 限定）。**自前ロールのARNを `CreatePaymentManager` に渡す場合、これらはAWS側で自動付与されない**（コンソールでロールを自動生成させた場合のみ付与）。付与漏れがあると `CreatePaymentManager` が `The execution role is missing the required permission: bedrock-agentcore:CreateWorkloadIdentity` で失敗する（AWS公式ドキュメント "Using an existing role" 参照）。
